...
protocol SFAlbumPickerViewModelProtocol:NSObjectProtocol {
    ...
    /// 相册数据发生了变化的时候及时通知UI刷新
    /// - Parameter viewModel: 模型
...
class SFAlbumPickerViewModel: NSObject,PHPhotoLibraryChangeObserver {
    ...
    /// 加载某个特定的相簿
    /// - Parameter collection: 某相簿
    /// - Returns: 空
    private func loadSpecificCollection(_ collection:PHAssetCollection?) -> Void {
        var fetchResult:PHFetchResult<PHAsset> = PHAsset.fetchAssets(with: self.fetchOption)
        if collection != nil {
            fetchResult = PHAsset.fetchAssets(in: collection!, options: self.fetchOption)
        }
    ...
    internal func loadAllPhotos() -> Void {
        ...
        DispatchQueue.global().async {
            self.currentAssetsCollection = nil
            self.loadSpecificCollection(nil)
        }
    }
    ...
    internal func loadSpecificCollection(_ model:SFAlbumPickerCollectionTableViewCellModel) -> Void {
        DispatchQueue.global().async {
            ...
            self.currentAssetsCollection = model.associatedCollection!
            self.loadSpecificCollection(model.associatedCollection!)
        }
    }
    // MARK: - actions
    // MARK: - accessors
    ...
    private var currentAssetsCollection:PHAssetCollection?
    ...
    func photoLibraryDidChange(_ changeInstance: PHChange) {
        self.loadSpecificCollection(self.currentAssetsCollection)
